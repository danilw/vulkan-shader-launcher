...
#ifdef YARIV_SHADER
        retval = vk_load_shader_yariv(dev, (const uint32_t *)buf_vs_code, &render_data->shaders[SHADER_BUF_VERTEX].shader, sizeof(buf_vs_code));
        if (!vk_error_is_success(&retval))
		{
			vk_error_printf(&retval, "Could not load the shaders\n");
			return retval;
		}
        retval = vk_load_shader_yariv(dev, (const uint32_t *)buf_fs_code, &render_data->shaders[SHADER_BUF_FRAGMENT].shader, sizeof(buf_fs_code));
        if (!vk_error_is_success(&retval))
		{
			vk_error_printf(&retval, "Could not load the shaders\n");
			return retval;
...
static bool render_loop_buf(struct vk_physical_device *phy_dev, struct vk_device *dev, struct vk_render_essentials *essentials,
		struct render_data *render_data, VkCommandBuffer cmd_buffer, int render_index, struct app_data_struct *app_data)
{
	vk_error retval = VK_ERROR_NONE;
	VkResult res;
...
#ifdef NO_RESIZE_BUF
	update_push_constants_local_size(render_data->buf_obuffers[render_index].surface_size.width, render_data->buf_obuffers[render_index].surface_size.height);
#endif
    ...
    if(render_index_t<0)render_index_t=1;
	...
	vkCmdBindDescriptorSets(cmd_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
			render_data->buf_layout.pipeline_layout, 0, 1, &render_data->buf_desc_set, 0, NULL);
...
static bool render_loop_draw(struct vk_physical_device *phy_dev, struct vk_device *dev, struct vk_swapchain *swapchain, struct app_os_window *os_window)
{
	...
	if (!render_loop_buf(phy_dev, dev, &essentials, &render_data, offscreen_cmd_buffer, render_index, &os_window->app_data)){
	...
	if(render_index>=1)render_index=0;else render_index+=1;
	return true;
