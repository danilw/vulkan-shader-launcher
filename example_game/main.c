        ...
        struct vk_swapchain *swapchain, struct vk_render_essentials *essentials, struct render_data *render_data, bool reload_shaders)
{
    ...
    if((!load_once)||(reload_shaders)){
        ...
        render_data->shaders[SHADER_MAIN_VERTEX] = (struct vk_shader){
            .spirv_file = "shaders/spv/main.vert.spv", //use files if YARIV_SHADER not defined
...
#ifdef YARIV_SHADER
        retval = vk_load_shader_yariv(dev, (const uint32_t *)vs_code, &render_data->shaders[SHADER_MAIN_VERTEX].shader, sizeof(vs_code));
        if (!vk_error_is_success(&retval))
        {
            vk_error_printf(&retval, "Could not load the shaders\n");
            return retval;
        }
        retval = vk_load_shader_yariv(dev, (const uint32_t *)fs_code, &render_data->shaders[SHADER_MAIN_FRAGMENT].shader, sizeof(fs_code));
        if (!vk_error_is_success(&retval))
        {
            vk_error_printf(&retval, "Could not load the shaders\n");
            return retval;
    ...
    retval = vk_make_graphics_pipelines(dev, &render_data->main_pipeline, 1, true); //true to enable blending
    if (!vk_error_is_success(&retval))
    {
...
void update_game(struct app_data_struct *app_data){
    update_time(app_data->iTime);
    update_char_anim();
    
}

void update_push_constants(struct shaders_uniforms *push_constants, struct app_os_window *os_window, int id, float *p_pos){
    push_constants->draw_id[0]=id;
    push_constants->draw_id[1]=id;
    push_constants->draw_pos_g[0]=p_pos[0];
    push_constants->draw_pos_g[1]=p_pos[1];
    push_constants->FPS=(int)(1.0/os_window->app_data.iTimeDelta);
    push_constants->hpdata[0]=uniform_vals.u_hpdata[0];
    push_constants->hpdata[1]=uniform_vals.u_hpdata[1];
    push_constants->hpdata[2]=uniform_vals.u_hpdata[2];
    push_constants->hpdata[3]=os_window->app_data.pause ? 10 : 0;
    push_constants->u_valt[0]=uniform_vals.u_valt[0];
    push_constants->u_valt[1]=uniform_vals.u_valt[1];
    push_constants->u_dataxx[0]=uniform_vals.u_dataxx[0];
    push_constants->u_dataxx[1]=uniform_vals.u_dataxx[1];
    push_constants->u_dataxx[2]=uniform_vals.u_dataxx[2];
    push_constants->u_dataxx[3]=uniform_vals.u_dataxx[3];
}
...
void update_params(struct app_data_struct *app_data, bool fps_lock){
    ...
    update_mouse(app_data->iMouse_click[0],app_data->iMouse_click[1]);
    update_game(app_data);
}
...
static bool render_loop_draw(struct vk_physical_device *phy_dev, struct vk_device *dev, struct vk_swapchain *swapchain, struct app_os_window *os_window)
{
    ...
    float tw=os_window->app_data.iResolution[0];
    float th=os_window->app_data.iResolution[1];
    //background
    {
        float p_pos[2]={0};
        ...
        update_push_constants(&render_data.push_constants, os_window, this_id, p_pos);
        VkRect2D scissor;
        memset(&scissor, 0, sizeof(scissor));
        scissor.extent.width = tw - (tw - ts[0] * th);
        scissor.extent.height = th - (th - ts[1] * th);
        scissor.offset.x = 0 + (tw / 2. - ts[0] / 2.*th - p_pos[0] * th);
        scissor.offset.y = 0 + (th / 2. - ts[1] / 2.*th + p_pos[1] * th);
        scissor.offset.x=(scissor.offset.x<0)?0:scissor.offset.x;
        scissor.offset.y=(scissor.offset.y<0)?0:scissor.offset.y;
    ...
    //debug
    if (os_window->app_data.drawdebug) {
        float p_pos[2] = { -0.7,-0.3 };
        int this_id = debug;
        float ts[2];
        ts[0] = uniform_vals.obj_size[this_id][0];
        ts[1] = uniform_vals.obj_size[this_id][1];
    ...
    for (int i = 0.; i<20; i++) {
        ...
        {
            float p_pos[2];
            p_pos[0] = uniform_vals.p_pos[i][0];
            p_pos[1] = uniform_vals.p_pos[i][1];
...
void init_win_params(struct app_os_window *os_window){
    ...
    os_window->app_data.iTimeDelta=1.0/60.0;
